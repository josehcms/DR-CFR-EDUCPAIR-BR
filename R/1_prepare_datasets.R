...
### R Script 1: read and process raw data files, adjust variables and prepare
###             database for anlysis
...
datBRMatched <- 
  do.call(
    rbind,
    lapply(
      c( 1970, 1980, 1991, 2000, 2010 ), # loop through each census data set
      function( x ){
        
        gc( reset = TRUE )
        
        load( paste0( 'DATA/IPUMS/BR', x, '.RData' ) )
...
### 3. Prepare data set for all females #---------------------------------------
